...
import { Expense, Customer } from '../../types/database';
import { Modal } from '../ui/Modal';
import { ConfirmDialog } from '../ui/ConfirmDialog';

export function Expenses() {
  const [receivables, setReceivables] = useState<(Expense & { supplier?: Customer })[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      const [receivablesData, suppliersData] = await Promise.all([
        supabase
          .from('expenses')
          .select('*, suppliers(*)')
  ...
  const totalReceived = receivables.filter((r) => r.status === 'paid').reduce((sum, r) => sum + Number(r.amount), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-green-500">
          ...
          <p className="text-xs text-gray-500 mt-1">{receivables.filter((r) => r.status === 'paid').length} contas</p>
        </div>
                  ...
                  <tr key={receivable.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-700">{receivable.supplier?.name || 'N/A'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
