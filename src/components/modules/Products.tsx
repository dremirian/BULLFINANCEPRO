...
export function Products() {
  const [receivables, setReceivables] = useState<(Product & { customer?: Customer })[]>([]);
  const [products, setCustomers] = useState<Customer[]>([]);
  ...
  const [formData, setFormData] = useState({
    ...
    price: '',
    cost: '',
    status: 'inactive' as 'inactive' | 'active' | 'overdue',
    unit: '',
    notes: '',
  });
  ...
  const loadData = async () => {
    try {
      ...
      const [receivablesData, productsData] = await Promise.all([
        supabase
          .from('products')
          .select('*, products(*)')
      ...
      setReceivables((receivablesData.data || []).map((item: any) => ({
        ...item,
        customer: item.products,
      })));
      setCustomers(productsData.data || []);
    } catch (error) {
  ...
  const totalReceived = receivables.filter((r) => r.status === 'active').reduce((sum, r) => sum + Number(r.amount), 0);
  const totalPending = receivables.filter((r) => r.status === 'inactive').reduce((sum, r) => sum + Number(r.amount), 0);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Gerencie suas seu estoque e produtos</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-green-500">
          ...
          <p className="text-xs text-gray-500 mt-1">{receivables.filter((r) => r.status === 'active').length} contas</p>
        </div>

        <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-yellow-500">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm text-gray-600">Inativo</p>
            <AlertCircle className="text-yellow-500" size={24} />
          </div>
          <p className="text-2xl font-bold text-yellow-600">
            {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalPending)}
          </p>
          <p className="text-xs text-gray-500 mt-1">{receivables.filter((r) => r.status === 'inactive').length} contas</p>
        </div>
                  ...
                  <tr key={receivable.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-700">{new Date(receivable.price).toLocaleDateString('pt-BR')}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Data de Preço *</label>
              <input
                type="date"
                required
